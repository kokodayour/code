...
# use haversine formula to get the distance(unit: m) between two points
...
# batch preprocessing data from different cities
def pre_data():
    url = "E:/LAND USE/data/"
...
# round up the number to 5 decimal places
def get_ceilnum(num):
    return ceil(num * 100000) / 100000

# round down the number to 5 decimal places
...
def get_building_map(url, city_name):
    ...
    file = Path(__file__).parent.parent / "data" / city_name / f"{city_name}_landuse.csv"
    building_map = {}
    data = pd.read_csv(file, encoding='utf-8')
    for i in range(len(data)):
...
def get_type2id():
    ...
    file = Path(__file__).parent.parent / "data" / "Lisboa" / "Lisboa_landuse.csv"
    data = pd.read_csv(file, encoding='utf-8')
    type2id = {}
    type_list = []
    n = 1
...
def save_cities_cell(city:list,length:list):
    # url = "E:/LAND USE/data/"
    url = Path(__file__).parent.parent / "data"
    type2id = get_type2id()
    for city_name in city:
        for i in length:
