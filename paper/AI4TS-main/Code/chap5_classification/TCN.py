...
class TCNClassifier(nn.Module):
    """TCN 时间序列分类器"""
    def __init__(self, input_length, num_classes, num_channels=[32, 64, 128], kernel_size=2, dropout_rate=0.2):
        ...
        for i in range(num_layers):
            ...
            # 定义一层带因果卷积的残差块
            layers.append(ResidualBlock(in_channels, out_channels, kernel_size, stride=1, dropout=dropout_rate))
...
def main():
    ...
    patience = 20  # 早停耐心值
    val_ratio = 0.2  # 验证集比例
    model_type = "tcn"  # 使用TCN模型
    dropout_rate = 0.3  # Dropout率
    num_channels = [32, 64, 128]  # TCN通道数
    kernel_size = 3  # TCN卷积核大小
    ...
    # 使用TCN模型
    model = TCNClassifier(input_length, num_classes, num_channels, kernel_size, dropout_rate).to(device)
    print("使用TCN模型")
