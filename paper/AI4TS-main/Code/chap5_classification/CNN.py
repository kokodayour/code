...
class CNNTSClassifier(nn.Module):
    """CNN时间序列分类器"""
    def __init__(self, input_length, num_classes, dropout_rate=0.5):
        super(CNNTSClassifier, self).__init__()
        ...
        self._get_conv_output(( 1, input_length))
...
class ResidualCNNTSClassifier(nn.Module):
    """带残差连接的CNN时间序列分类器"""
    def __init__(self, input_length, num_classes, dropout_rate=0.3):
        super(ResidualCNNTSClassifier, self).__init__()
        ...
        # 残差块1
        self.resblock1 = ResidualBlock(64, 128, stride=2, dropout=dropout_rate)
        
        # 残差块2
        self.resblock2 = ResidualBlock(128, 256, stride=2, dropout=dropout_rate)
        
        # 残差块3
        self.resblock3 = ResidualBlock(256, 512, stride=2, dropout=dropout_rate)
...
def main():
    ...
    patience = 20  # 早停耐心值
    val_ratio = 0.2  # 验证集比例
    model_type = "cnn"  # 可选: "cnn" 或 "residual_cnn"
    ...
    if model_type == "residual_cnn":
        model = ResidualCNNTSClassifier(input_length, num_classes, dropout_rate).to(device)
        print("使用残差CNN模型")
    else:
        model = CNNTSClassifier(input_length, num_classes, dropout_rate).to(device)
        print("使用标准CNN模型")
