...
class ReconstructionAE(nn.Module):
    def __init__(self, seq_len, input_dim, hidden_dims=[256, 128, 64]):
        super(ReconstructionAE, self).__init__()
        self.seq_len = seq_len
    ...
    def forward(self, x):
        batch_size = x.size(0)
        x = x.reshape(batch_size, -1)  # 展平 (batch, seq_len * input_dim)
        z = self.encoder(x)
        out = self.decoder(z)
        out = out.view(batch_size, self.seq_len, self.input_dim)
        return out
...
model = ReconstructionAE(seq_len, input_dim).to(device)
